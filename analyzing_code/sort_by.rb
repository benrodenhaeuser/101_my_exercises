...
# One way to think of what sort_by is doing is like this: It (1) maps the array to an array of pairs, (2) sorts that array by the second component of each pair (relying on `<=>`), and (3) then projects each pair onto its first component.

# sort_by makes life very easy for us because all we have to is what the "mapping" in step 1 should look like. To specify this, we can either pass a block or a method symbol to sort_by, as seen above.

# We can actually mimick the three steps in Ruby code as follows. For our running example, this looks like this:
...
# We can even implement our own sort_by functionality in this way:
