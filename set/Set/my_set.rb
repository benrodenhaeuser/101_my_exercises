...
class MySet
  ...
  def initialize(elems = [])
    @hash = Hash.new.merge!(elems.map { |elem| [elem, 1] }.to_h)
  end
  ...
  # (Note that the Set class does not require `enum.each` to return
  # an Enumerator, like we do below.)
