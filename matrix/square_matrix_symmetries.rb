...
- We represent the matrix here as an array of length n^2, because that turned
  out to be convenient for tic tac toe.

There are 7 symmetries in a square matrix:
...
In tic tac toes, symmetric boards are strategically equivalent. So breaking these symmetries is interesting from an efficiency point of view.

=end

# ----------------------------------------------------------------------------
# SYMMETRIES IN A SQUARE MATRIX
# ----------------------------------------------------------------------------
...
Benchmark.bmbm do |x|
  ...
  x.report("rot180") { rotate180(state) }
  x.report("rot270") { rotate270(state) }
  x.report("trans") { transpose(state) }
  x.report("sec_dg") { reflect_sec_diag(state) }
